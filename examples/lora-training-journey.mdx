Learn how to train custom LoRA (Low-Rank Adaptation) models using the Weights API. This guide walks you through creating personalized AI models that can generate images in your specific style or concept.
...
For advanced users, you can download your trained LoRA model files:
...
const characterModel = await client.imageLoraModels.create({
  ...
  validationPrompt: "A portrait of Luna in different styles",
  isPublic: false, // Keep personal characters private
});
...
const styleModel = await client.imageLoraModels.create({
  ...
  validationPrompt: "A landscape in cyberpunk style",
  isPublic: true, // Share artistic styles
});
```
...
- Explore [Image Generation](./image-generation-journey.mdx) to use your trained models
- Learn about [RVC Voice Models](./rvc-voice-journey.mdx) for voice synthesis
- Check out [Song Generation](./song-generation-journey.mdx) for audio content
